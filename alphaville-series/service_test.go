...
func TestWriteService_EmptyDB(t *testing.T) {
	defer concepts.CleanTestDB(t, db, basicAsUUID)
	...
	tests := []struct {
		...
		{
			testName:        "Basic TME Alphaville Series is successful and can be read from DB",
			filePathToWrite: "./fixtures/write/draaisma_1tme.json",
			filePathToRead:  "./fixtures/read/draaisma_1tme.json",
			conceptUUID:     basicAsUUID,
			expectedError:   "",
			updatedConcepts: concepts.ConceptChanges{
				UpdatedIds: []string{basicAsUUID},
		...
		{
			testName:        "Concorded Alphaville Series fails",
			filePathToWrite: "./fixtures/write/concordedAs.json",
			filePathToRead:  "",
			conceptUUID:     invalidPayloadUUID,
			expectedError:   "alphaville-series do not currently support concordance",
			updatedConcepts: concepts.ConceptChanges{
				UpdatedIds:     []string{},
				ChangedRecords: []concepts.Event{},
			},
