...
func (s *ConceptService) Read(uuid string, transID string) (interface{}, bool, error) {
	...
	query := &neoism.CypherQuery{
		Statement: `
			...
			WITH
				collect(broader.uuid) as broaderUUIDs,
				canonical,
				issuer,
				org,
				parent,
				person,
				collect(related.uuid) as relatedUUIDs,
				collect(supersededBy.uuid) as supersededByUUIDs,
				collect(focused.uuid) as focusedUUIDs,
				role,
				roleRel,
				parentOrg,
				coo,
