...
var conceptsDriver *SubjectService

const (
	testTID            = "tid_1234test"
	commoditiesUUID    = "9dcc589d-a118-30a7-9de9-cfc4309f42b3"
...
func init() {
	...
	conceptsDriver = NewSubjectService(db)
...
func TestWriteService_EmptyDB(t *testing.T) {
	defer concepts.CleanTestDB(t, db, commoditiesUUID)
	...
	tests := []struct {
		...
		{
			testName:        "Basic TME Subject is successful and can be read from DB",
			filePathToWrite: "./fixtures/write/commodities_1tme.json",
			filePathToRead:  "./fixtures/read/commodities_1tme.json",
			conceptUUID:     commoditiesUUID,
			expectedError:   "",
			updatedConcepts: concepts.ConceptChanges{
				UpdatedIds: []string{commoditiesUUID},
				ChangedRecords: []concepts.Event{
					{
						ConceptUUID:   commoditiesUUID,
						ConceptType:   "Subject",
						AggregateHash: "13142572839650193111",
		...
		{
			testName:        "Concorded Subject fails",
			filePathToWrite: "./fixtures/write/concordedSubject.json",
			filePathToRead:  "./fixtures/write/concordedSubject.json",
			conceptUUID:     invalidPayloadUUID,
			expectedError:   "subjects do not currently support concordance",
			updatedConcepts: concepts.ConceptChanges{
				UpdatedIds:     []string{},
				ChangedRecords: []concepts.Event{},
			},
