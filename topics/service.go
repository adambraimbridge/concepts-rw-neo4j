...
func NewTopicService(db neoutils.NeoConnection) *TopicService {
	return &TopicService{db}
}

func (ts *TopicService) Write(thing interface{}, transID string) (interface{}, error) {
	...
	existingConcept, exists, err := ts.Read(aggregatedConceptToWrite.PrefUUID, transID)
	if err != nil {
		logger.WithError(err).WithTransactionID(transID).WithUUID(aggregatedConceptToWrite.PrefUUID).Error("read request for existing concordance resulted in error")
		return updateRecord, err
	}
	...
	if exists {
		...
		if len(conceptsToTransferConcordance) > 0 {
			prefUUIDsToBeDeletedQueryBatch, err = concepts.HandleTransferConcordance(conceptsToTransferConcordance, ts.conn, &updateRecord, hashAsString, aggregatedConceptToWrite.PrefUUID, transID)
			if err != nil {
				return updateRecord, err
			}
		...
		prefUUIDsToBeDeletedQueryBatch, err = concepts.HandleTransferConcordance(sourceUuidsAndTypes, ts.conn, &updateRecord, hashAsString, aggregatedConceptToWrite.PrefUUID, transID)
		if err != nil {
			return updateRecord, err
		}
	...
	if err = ts.conn.CypherBatch(queryBatch); err != nil {
...
func clearDownExistingNodes(ac concepts.AggregatedConcept) []*neoism.CypherQuery {
	...
	for _, sr := range ac.SourceRepresentations {
		deletePreviousSourceIdentifiersLabelsAndPropertiesQuery := &neoism.CypherQuery{
			...
			OPTIONAL MATCH (t)-[brd:BROADER]->()
			OPTIONAL MATCH (t)-[rel:IS_RELATED_TO]->()
			REMOVE t:%s
			SET t={uuid:{id}}
			DELETE iden, i, eq, brd, rel`, concepts.GetLabelsToRemove()),
...
func (ts *TopicService) Read(uuid string, transID string) (interface{}, bool, error) {
	...
	err := ts.conn.CypherBatch([]*neoism.CypherQuery{query})
	if err != nil {
